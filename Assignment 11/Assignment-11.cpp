...
float TotalAvgP(BSearchTree Root)
{
    PNode p = Root;
...
void InOrderTraversal(BSearchTree Root) {
    if (Root == NULL) return;
    InOrderTraversal(Root->LP);
    show(Root);
    InOrderTraversal(Root->RP);
}
...
int main()
{
    ...
    InitBST(T);
    InsertT(T, 3, "Le Van A", "ET1-02", 8.0);
    InsertT(T, 4, "Nguyen Van B", "ET1-04", 10.0);
    InsertT(T, 2 , "Trinh Thi C", "ET1-03", 9.0);
    InsertT(T, 5, "Do Quang D", "ET1-05", 7.0);
    InsertT(T, 1, "Dao Thi E", "ET1-06", 8.5);
    cout << "danh sach sinh vien: " << endl; InOrderTraversal(T);
    cout << "\nnhap mssv ban muon xoa: "; cin >> y;
    DeleteT(T, y);
    cout << "sinh vien vua nhap da duoc xoa";
    cout << "\ndanh sach sinh vien: " << endl; InOrderTraversal(T);
    cout << "\ndiem trung binh cua toan bo sinh vien trong danh sach: "; cout << TotalAvgP(T) << endl;
    cin.ignore();
